...
def main():
    parser = argparse.ArgumentParser(description="Kubernetes Python Client Extended Utility")
    parser.add_argument('action', choices=[
        'list_pods', 'list_services', 'create_ns', 'delete_ns',
        'create_deploy', 'scale_deploy', 'create_svc',
        'watch_events', 'get_logs'
    ], help="Action to perform")
    parser.add_argument('--name', help="Name of the resource")
    parser.add_argument('--namespace', default='default', help="Kubernetes namespace")
    parser.add_argument('--image', help="Container image for deployment")
    parser.add_argument('--replicas', type=int, help="Number of replicas")
    parser.add_argument('--port', type=int, help="Service port")
    parser.add_argument('--target_port', type=int, help="Service target port")
    parser.add_argument('--tail', type=int, default=50, help="Number of log lines to fetch")
    ...
    elif args.action == 'create_ns':
        create_namespace(args.name)
    elif args.action == 'delete_ns':
        delete_namespace(args.name)
    elif args.action == 'create_deploy':
        create_deployment(args.name, args.image, args.replicas, args.namespace)
    elif args.action == 'scale_deploy':
        scale_deployment(args.name, args.replicas, args.namespace)
    elif args.action == 'create_svc':
        create_service(args.name, args.port, args.target_port, args.namespace)
    elif args.action == 'watch_events':
        watch_events(args.namespace)
    elif args.action == 'get_logs':
        get_pod_logs(args.name, args.namespace, args.tail)
    else:
